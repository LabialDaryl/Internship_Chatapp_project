...
## Phase Summary — All Phases Completed & Verified (Phases 1 – 6)

| Phase | Description | Key Deliverables | Status |
|:---|:---|:---|:---|
| **Phase 1** | Authentication & Sanctum API | Login, 5-Field Register (`name`, `username`, `email`, `password`, `date_of_birth`), Sanctum Token Auth, User Profile | ✅ Completed |
| **Phase 2** | Conversations & Messaging Schema | PostgreSQL Migrations (`conversations`, `conversation_participants`, `messages`, `read_receipts`), REST Controllers | ✅ Completed |
| **Phase 3** | Frontend Design System | Tailwind CSS v3.4, Vibrant Violet dark theme, Glassmorphism, Responsive Composables (`useTheme`, `useBreakpoints`) | ✅ Completed |
...
- **Pre-populated Accounts** (Password: `password`):
  - Daryl (`daryl@example.com`, `@daryl`)
  - Alice Vance (`alice@example.com`, `@alice`)
  - Bob Miller (`bob@example.com`, `@bob`)
...
| `POST` | `/api/login` | Authenticate user & issue token | Public |
| `GET` | `/api/user` | Get authenticated profile | `auth:sanctum` |
| `PUT` | `/api/user/profile` | Update profile information | `auth:sanctum` |
| `PUT` | `/api/user/password` | Update account password | `auth:sanctum` |
...
## Running the Complete Development Server Stack

To run the application locally, start the following 3 processes:
...
# Terminal 3 — Vue 3 Frontend Server
cd frontend
npm run dev
```
