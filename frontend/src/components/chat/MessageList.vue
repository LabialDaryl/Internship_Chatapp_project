<template>
  <div class="flex-1 flex flex-col min-h-0 relative overflow-hidden bg-slate-950">
      ...
      >
        ...
        <template v-else>
          ...
          <div v-else-if="msg.type === 'file'"
            :class="[
              'px-4 py-3 rounded-2xl text-sm flex items-center space-x-3 border shadow-sm relative',
              isOwn(msg)
          ...
          >
            ...
            <div class="flex-1 min-w-0">
              <p class="text-xs font-semibold truncate flex items-center space-x-1">
                <span>{{ getFileName(msg.body) }}</span>
                <span v-if="msg.is_pinned" class="text-amber-400 text-xs">📌</span>
              </p>
    ...
    <!-- Read Receipts Breakdown Modal -->
    <ReadReceiptsModal
      :show="showReadReceiptsModal"
      :messageId="selectedReceiptMessageId"
      @close="showReadReceiptsModal = false"
    />
  </div>
</template>
...
import PinnedMessagesBanner from './PinnedMessagesBanner.vue'
...
const triggerPin = async (msg) => {
  activeDropdownId.value = null
  if (!chatStore.activeConversation) return
  await messagesService.togglePinMessage(chatStore.activeConversation.id, msg.id)
  msg.is_pinned = !msg.is_pinned
  chatStore.showToast(msg.is_pinned ? 'Message pinned!' : 'Message unpinned!')
}

const handleUnpinMessage = async (msg) => {
  if (!chatStore.activeConversation || !msg) return
  await messagesService.togglePinMessage(chatStore.activeConversation.id, msg.id)
  msg.is_pinned = false
  chatStore.showToast('Message unpinned!')
}
...
const openReadReceiptsModal = (msgId) => {
  selectedReceiptMessageId.value = msgId
  showReadReceiptsModal.value = true
}
